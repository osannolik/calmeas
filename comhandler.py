...
class ComHandler():
    ...
    def handler_Rx(self, block=True, timeout=None):
        try:
            ...
            # self._bytelen.append( len(bytes) )
            # self._framelen.append( len(full_frames) )
            # if len(self._time)>2000:
            #     f = open('debug.txt','w')
            #     f.write(','.join(map(str, self._time)) + '\n')
            #     f.write(','.join(map(str, self._bytelen)) + '\n')
            #     f.write(','.join(map(str, self._framelen)))
            #     f.close()
            #     self._time = list()
            #     self._bytelen = list()
            #     self._framelen = list()
            #     print "Printed frame times to file"
    ...
    def handler_Tx(self, block=False, timeout=None):
        try:
            ...
            #print 'Putting ' + str(bytes) + ' ' + str(type(bytes)) + ' to byte queue'
