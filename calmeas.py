...
class Symbol():
    ...
    def initDataBuffer(self, size=10000):
        #self.data = np.zeros(1, dtype=self._datatype.np_basetype)
    ...
    def setPeriod(self, p):
        ...
        #self.time = np.arange(len(self.data)) * p
...
class CalMeas():
    ...
    def interfaceCallback(self, f):
        ...
        #print f.FrameBytesFormatted()
        #print '0x{:x} ({})'.format(f.data.var_uint32[0], f.data.var_uint32[0])
    ...
    def ID_Raster_Callback(self, f):
        ...
        #logging.debug(rasterIndex)

        for i, symbolName in enumerate(self.raster[rasterIndex]):
            val = data[i].value
            #logging.debug('{} = {}'.format(symbolName, val))
            self.targetSymbols[symbolName].setValue(val)



        # if (time() - self.tick) > 2*self.rasterPeriods[rasterIndex]:
        #     print "Unexpected delay"
        #     self._updateTimes.append( time() - self.tick )
        # #self._updateTimes.append( time() - self.tick )

        # if len(self._updateTimes)>=20:
